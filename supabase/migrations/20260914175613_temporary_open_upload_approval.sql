-- Temporary prototype rule: all active people may approve the latest upload.
-- Requests/change requests retain their existing contribution and assignment rules.
...
declare d public.documents; v public.document_versions; a public.approvals; result_id uuid; target uuid; begin
 ...
 if not exists(select 1 from public.people where id=p_actor and (p_decision='approved' or role in ('admin','contributor')) and deactivated_at is null) or exists(select 1 from public.projects where id=d.project_id and status='closed') then raise exception 'This review is read-only.'; end if;
 ...
 -- Repeated approval must preserve the original approver and audit event.
 if p_decision='approved' and a.status='approved' then return a.id; end if;
 ...
 if p_decision='requested' then
  ...
  if not exists(select 1 from public.people where id=p_reviewer and deactivated_at is null) then raise exception 'Choose a reviewer who can approve documents.'; end if;
  ...
  if a.id is null then
   insert into public.approvals(document_version_id,requested_by,status,decided_by,decided_at,decision_note)
    values(v.id,null,'approved',p_actor,clock_timestamp(),nullif(btrim(p_note),'')) returning id into result_id;
   target=v.uploaded_by;
  else
   update public.approvals set status='approved',decided_by=p_actor,decided_at=clock_timestamp(),decision_note=nullif(btrim(p_note),'') where id=a.id returning id into result_id;
