...
begin
 ...
 for op in select value from jsonb_array_elements(p_operations) loop
  ...
  if op->>'action'='task' then
   ...
   if newrow->>'status'='done' and exists(select 1 from public.documents d where d.task_id=tid and coalesce(d.requires_approval,true) and d.deleted_at is null and coalesce((
